...
<div class="min-h-screen bg-[#f9f7f5]">
	<header class="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-200 shadow-sm">
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-4">
			<button onclick={() => history.back()} class="p-2 text-gray-500 hover:text-orange-500 hover:bg-gray-100 rounded-lg transition" title="Back">
	...
	<div class="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
		<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
			...
			<div class="px-6 py-6">
				<div class="flex flex-col sm:flex-row gap-6 items-center justify-center">
					<div class="flex-1 w-full">
						<div class="flex justify-between items-center mb-2">
							<span class="text-sm font-medium text-gray-500">Competitors</span>
					...
					<div class="flex-1 w-full">
						<div class="flex justify-between items-center mb-2">
							<span class="text-sm font-medium text-gray-500">U-DO</span>
				...
				<div class="mt-6 text-center">
					<span class="inline-flex items-center gap-2 bg-green-50 text-green-700 px-5 py-2.5 rounded-full text-lg font-bold">
						You save <span class="text-2xl">15%</span> on every order
					</span>
				</div>
			</div>
		</div>
				...
				{#if orders > 0 && orderVal > 0 && savingsRate > 0}
					<div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
						<div class="bg-orange-50 rounded-xl p-4 text-center">
							<p class="text-xs font-medium text-orange-600 uppercase tracking-wide mb-1">Daily</p>
							<p class="text-2xl font-bold text-orange-600">{formatCurrency(dailySavings)}</p>
						</div>
						<div class="bg-orange-50 rounded-xl p-4 text-center">
							<p class="text-xs font-medium text-orange-600 uppercase tracking-wide mb-1">Weekly</p>
							<p class="text-2xl font-bold text-orange-600">{formatCurrency(weeklySavings)}</p>
						</div>
						<div class="bg-green-50 rounded-xl p-4 text-center ring-2 ring-green-200">
							<p class="text-xs font-medium text-green-600 uppercase tracking-wide mb-1">Monthly</p>
							<p class="text-2xl font-bold text-green-600">{formatCurrency(monthlySavings)}</p>
							<p class="text-[10px] text-green-500 mt-0.5">30 days</p>
						</div>
						<div class="bg-green-50 rounded-xl p-4 text-center">
							<p class="text-xs font-medium text-green-600 uppercase tracking-wide mb-1">Yearly</p>
							<p class="text-2xl font-bold text-green-600">{formatCurrency(yearlySavings)}</p>
							<p class="text-[10px] text-green-500 mt-0.5">365 days</p>
						</div>
					</div>
				{:else}
		...
		<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
				...
				<table class="w-full">
					<thead>
						<tr class="bg-gray-50">
							...
							<th class="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Daily Orders</th>
							<th class="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Daily Savings</th>
							<th class="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Weekly Savings</th>
							<th class="text-right px-4 py-3 text-xs font-semibold text-orange-500 uppercase tracking-wide">Monthly (30 Days)</th>
							<th class="text-right px-4 py-3 text-xs font-semibold text-green-600 uppercase tracking-wide">Yearly (365 Days)</th>
						</tr>
					</thead>
					<tbody class="divide-y divide-gray-50">
						{#each BENCHMARK_DATA as row}
							...
							<tr class="hover:bg-orange-50/30 transition">
								...
								<td class="text-right px-4 py-4 text-gray-700 font-medium">{formatCurrency(weekly)}</td>
								<td class="text-right px-4 py-4 font-bold text-green-600">{formatCurrency(monthly)}</td>
								<td class="text-right px-4 py-4 font-bold text-green-700">{formatCurrency(yearly)}</td>
							</tr>
						{/each}
					</tbody>
				</table>
			...
			<div class="px-6 py-3 bg-gray-50 border-t border-gray-100">
				<p class="text-xs text-gray-400">💡 Savings calculated as the commission difference (30% − {udoRateInput}%) × daily orders × average order value.</p>
			</div>
		</div>
		...
		<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
			<div class="bg-gradient-to-r from-green-600 to-emerald-500 px-6 py-5 text-white text-center">
				<h3 class="text-xl sm:text-2xl font-bold">✅ The "Direct Pricing" Guarantee</h3>
			</div>
			<div class="px-6 py-8 space-y-6">
				<div class="flex gap-4 items-start">
					<span class="flex-shrink-0 w-10 h-10 rounded-full bg-red-100 text-red-600 flex items-center justify-center font-bold text-lg">⚠️</span>
					<div>
						<h4 class="font-bold text-gray-900 mb-1">The Problem</h4>
						<p class="text-gray-600 leading-relaxed">Uber often allows or encourages restaurants to inflate their menu prices by <span class="font-bold text-red-600">15–20%</span> to cover commissions.</p>
					</div>
				</div>
				<div class="flex gap-4 items-start">
					<span class="flex-shrink-0 w-10 h-10 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center font-bold text-lg">🤝</span>
					<div>
						<h4 class="font-bold text-gray-900 mb-1">The U-DO Move</h4>
						<p class="text-gray-600 leading-relaxed">You tell the merchant: <span class="italic">"I'm only charging you <span class="font-bold text-orange-600">18–20% commission</span> (down from 30%). In exchange, you must list your <span class="font-bold text-orange-600">true in-store prices</span> on U-DO."</span></p>
					</div>
				</div>
				<div class="flex gap-4 items-start">
					<span class="flex-shrink-0 w-10 h-10 rounded-full bg-green-100 text-green-600 flex items-center justify-center font-bold text-lg">💎</span>
					<div>
						<h4 class="font-bold text-gray-900 mb-1">The Value</h4>
						<p class="text-gray-600 leading-relaxed">The customer sees a burger for <span class="font-bold text-green-700">$12.00</span> on U-DO instead of <span class="font-bold text-red-600 line-through">$14.50</span> on Uber. This <span class="font-bold text-green-700">"Menu Parity"</span> is your strongest marketing tool.</p>
					</div>
				</div>
				<div class="mt-4 bg-gray-50 rounded-xl p-5 border border-gray-100">
					...
					<div class="flex flex-col sm:flex-row gap-4 items-center justify-center">
						<div class="flex-1 text-center bg-red-50 rounded-xl p-4 border border-red-100">
							<p class="text-sm font-medium text-gray-500 mb-1">Uber Eats</p>
							<p class="text-3xl font-bold text-red-500">$14.50</p>
							<p class="text-xs text-red-400 mt-1">Inflated price to cover 30% commission</p>
						</div>
						<div class="text-2xl text-gray-300 font-bold">→</div>
						<div class="flex-1 text-center bg-green-50 rounded-xl p-4 border border-green-200 ring-2 ring-green-200">
							<p class="text-sm font-medium text-gray-500 mb-1">U-DO</p>
							<p class="text-3xl font-bold text-green-600">$12.00</p>
							<p class="text-xs text-green-500 mt-1">True in-store price • Fair commission</p>
						</div>
					</div>
				</div>
			</div>
